...
@if($products->items()==null)
    ...
    <h1 class="result">Результаты поиска</h1>
    <div id="empty-search-results">
        <div class="message clearfix">
            <p>К сожалению, по запросу «<b>@if(isset($_GET['search'])){{$_GET['search']}}@endif</b>» мы ничего не смогли найти.</p>
        </div>
    </div>
...
<main>
    ...
    <div class='all  container '>
        <div class="products-page__title" data-role="header-block">
            <h1 class="title">Найдено:</h1>
            <span data-role="items-count" class="products-count">{{$products->total()}} товаров</span>

        </div>
        <div class='all   row'>
            <div class='in-all col-xl-'>
                <div class="filter ">
                    <form method="get" action="{{action('AllController@searchFilter')}}" >
                        <div>
                        ...
                        <div class='top col'>
                            <form style='margin: 0;' method='get' action="{{action('AllController@buy')}}">
                                ...
                                <div class='in-top'>
                                    ...
                                    <div class='in-top-price d-flex'>  <input type="hidden" name="price" value="{{$product->price}}">{{$product->price}}<i class='icon'></i></div>
                ...
                @endforeach
                    ...
                    @foreach($array as $arr)
                        @if(isset($_GET['search']))
                            <?php
                            $products->appends(['search' => $_GET['search']])
                            ?>
                           @endif
                    ...
                    @endforeach
                    {{$products->links()}}
               {{--     {{$searchwords->appends(['search' => $_GET['search']])->links()}}--}}
